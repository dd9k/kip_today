...
import '../../../widgetUtilities/superCalendar/SampleView.dart';
import '../../../widgetUtilities/superCalendar/GettingStartedCalendar.dart';
import '../../../widgetUtilities/superCalendar/model.dart';
import 'VisitorInviteNotifier.dart';

/// Render the widget of appointment editor calendar
class CalendarAppointmentEditor extends SampleView {
  static const String route_name = '/CalendarAppointmentEditor';
  /// creates the appointment editor
  CalendarAppointmentEditor(this.dataSource, this.calendarController, this.currentLang);
  ...
  @override
  _CalendarAppointmentEditorState createState() => _CalendarAppointmentEditorState();
}
...
class _CalendarAppointmentEditorState extends SampleViewState {
  ...
  String _subject = '';
  VisitorInviteNotifier _inviteNotifier;
  ...
  void initState() {
    _isMobile = false;
    calendarController = (widget as CalendarAppointmentEditor).calendarController;
    _view = calendarController.view;
    _appointments = _getAppointmentDetails();
    _events = (widget as CalendarAppointmentEditor).dataSource;
    _selectedColorIndex = 0;
    _subject = '';
    _inviteNotifier = Provider.of<VisitorInviteNotifier>(context, listen: false);
    super.initState();
  }
...
//            calendarController, _events, _onCalendarTapped, _onViewChanged, scheduleViewBuilder));
    final Widget _calendar = _getAppointmentEditorCalendar(
        calendarController, _events, _onCalendarTapped, _onViewChanged, scheduleViewBuilder);
    final double _screenHeight = MediaQuery.of(context).size.height;
    return Scaffold(
      resizeToAvoidBottomInset: false,
      body: calendarController.view == CalendarView.month && model.isWebFullView && _screenHeight < 800
          ? Scrollbar(
              isAlwaysShown: true,
  ...
  void _onViewChanged(ViewChangedDetails visibleDatesChangedDetails) {
    _inviteNotifier.getCalendarVisitorInvitation(calendarController.displayDate.month);
  ...
  void _onCalendarTapped(CalendarTapDetails calendarTapDetails) {
    ...
    InviteNewVisitor newAppointment;
    ...
    if (!model.isWebFullView && calendarController.view == CalendarView.month) {
      ...
      _inviteNotifier.updateCalendarView(CalendarView.day);
    } else {
      var isViewMode = true;
      var isEditOld = false;
      if (calendarTapDetails.appointments != null && calendarTapDetails.targetElement == CalendarElement.appointment) {
        ...
        if (appointment is InviteNewVisitor) {
          newAppointment = appointment;
          isViewMode = appointment.status != InvitationStatus.DRAFT;
          isEditOld = !isViewMode;
        }
      } else {
        isViewMode = false;
        newAppointment = InviteNewVisitor.init();
        newAppointment.startDate = Utilities().convertDateToString(calendarTapDetails.date, Constants.FORMAT_DATE_BASIC);
      }
      AppRouter.pushToWithArguments(context, DetailEventsPage.route_name, arguments: {
        'inviteOld': newAppointment,
        'isViewMode': isViewMode
      }, transitionType: TransitionType.inFromBottom, transitionDuration: Duration(milliseconds: 500)).then((value) {
        if (newAppointment.isSent && !isViewMode) {
          VisitorInviteNotifier inviteNotifier = Provider.of<VisitorInviteNotifier>(context, listen: false);
          if (isEditOld) {
            inviteNotifier.updateOldInvite(newAppointment);
          } else {
            inviteNotifier.addNewInvite(newAppointment);
          }
        }
      });
    }
      ...
      for (int day = -5; day < 5; day++) {
        for (int hour = 9; hour < 18; hour += 5) {
          appointmentCollection.add(Appointment(
            startTime: today.add(Duration(days: (month * 30) + day)).add(Duration(hours: hour)),
            endTime: today.add(Duration(days: (month * 30) + day)).add(Duration(hours: hour + 2)),
  ...
  SfCalendar _getAppointmentEditorCalendar(
    ...
    return SfCalendar(
        currentLang: (widget as CalendarAppointmentEditor).currentLang,
        firstDayOfWeek: 1,
...
String _getAppointmentTimeText(Appointment selectedAppointment) {
  if (selectedAppointment.isAllDay) {
    if (isSameDate(selectedAppointment.startTime, selectedAppointment.endTime)) {
      return DateFormat('EEEE, MMM dd').format(selectedAppointment.startTime).toString();
    }
    return DateFormat('EEEE, MMM dd').format(selectedAppointment.startTime) +
        ' - ' +
        DateFormat('EEEE, MMM dd').format(selectedAppointment.endTime).toString();
  } else if (selectedAppointment.startTime.day != selectedAppointment.endTime.day ||
      selectedAppointment.startTime.month != selectedAppointment.endTime.month ||
      selectedAppointment.startTime.year != selectedAppointment.endTime.year) {
    String endFormat = 'EEEE, ';
    if (selectedAppointment.startTime.month != selectedAppointment.endTime.month) {
      endFormat += 'MMM';
    }

    endFormat += ' dd hh:mm a';
    return DateFormat('EEEE, MMM dd hh:mm a').format(selectedAppointment.startTime) +
        ' - ' +
...
Widget displayAppointmentDetails(
  ...
  final Color defaultColor =
      model.themeData != null && model.themeData.brightness == Brightness.dark ? Colors.white : Colors.black54;

  final Color defaultTextColor =
      model.themeData != null && model.themeData.brightness == Brightness.dark ? Colors.white : Colors.black87;
  ...
  return ListView(padding: const EdgeInsets.all(0.0), children: <Widget>[
    ListTile(
      ...
      children: <Widget>[
                ...
                builder: (BuildContext context) {
                  return WillPopScope(
                      ...
                      child: Theme(
                        ...
                        child: AppointmentEditorWeb(model, selectedAppointment, colorCollection, colorNames, events,
                            timeZoneCollection, appointmentCollection),
                      ));
                });
          },
        ),
        IconButton(
          ...
          onPressed: () {
            events.appointments.removeAt(events.appointments.indexOf(selectedAppointment));
            events.notifyListeners(CalendarDataSourceAction.remove, <Appointment>[selectedAppointment]);
            Navigator.pop(context);
          },
        ),
        IconButton(
    ...
    ListTile(
        ...
        title: Text(selectedAppointment.subject.isNotEmpty ? selectedAppointment.subject : '(No Text)',
            style: TextStyle(fontSize: 20, color: defaultTextColor, fontWeight: FontWeight.w400)),
        subtitle: Padding(
          padding: const EdgeInsets.only(top: 5),
          child: Text(
            _getAppointmentTimeText(selectedAppointment),
            style: TextStyle(fontSize: 15, color: defaultTextColor, fontWeight: FontWeight.w400),
          ),
        )),
    if (selectedAppointment.resourceIds == null || selectedAppointment.resourceIds.isEmpty)
      Container()
    else
      ListTile(
        leading: Icon(
        ...
        title: Text(_getSelectedResourceText(selectedAppointment.resourceIds, events.resources),
            style: TextStyle(fontSize: 15, color: defaultTextColor, fontWeight: FontWeight.w400)),
      ),
    if (selectedAppointment.location == null || selectedAppointment.location.isEmpty)
      Container()
    else
      ListTile(
        leading: Icon(
...
String _getSelectedResourceText(List<Object> resourceIds, List<CalendarResource> resourceCollection) {
  String resourceNames;
  for (int i = 0; i < resourceIds.length; i++) {
    final String name =
        resourceCollection.firstWhere((CalendarResource resource) => resource.id == resourceIds[i]).displayName;
...
class _CalendarTimeZonePicker extends StatefulWidget {
  const _CalendarTimeZonePicker(this.backgroundColor, this.timeZoneCollection, this.selectedTimeZoneIndex, this.model,
      {@required this.onChanged});
...
class PopUpAppointmentEditor extends StatefulWidget {
  ...
  const PopUpAppointmentEditor(this.model, this.newAppointment, this.appointment, this.events, this.colorCollection,
      this.colorNames, this.selectedAppointment, this.timeZoneCollection);
...
class _PopUpAppointmentEditorState extends State<PopUpAppointmentEditor> {
  ...
  void _updateAppointmentProperties() {
    ...
    _selectedTimeZoneIndex =
        widget.selectedAppointment.startTimeZone == null || widget.selectedAppointment.startTimeZone == ''
            ? 0
    ...
    _selectedTimeZoneIndex = _selectedTimeZoneIndex == -1 ? 0 : _selectedTimeZoneIndex;
  ...
  @override
  Widget build(BuildContext context) {
    final Color defaultColor = widget.model.themeData != null && widget.model.themeData.brightness == Brightness.dark
        ? Colors.white
        : Colors.black54;

    final Color defaultTextColor =
        widget.model.themeData != null && widget.model.themeData.brightness == Brightness.dark
            ? Colors.white
            : Colors.black87;
    ...
    final Widget _startDatePicker = RawMaterialButton(
      ...
      child: Text(DateFormat('MMM dd, yyyy').format(_startDate),
          style: TextStyle(fontWeight: FontWeight.w500, color: defaultTextColor), textAlign: TextAlign.left),
    );
      ...
      onPressed: () async {
        ...
        if (time != null && time != _startTime) {
          setState(() {
            ...
            final Duration difference = _endDate.difference(_startDate);
            _startDate =
                DateTime(_startDate.year, _startDate.month, _startDate.day, _startTime.hour, _startTime.minute, 0);
            _endDate = _startDate.add(difference);
            _endTime = TimeOfDay(hour: _endDate.hour, minute: _endDate.minute);
          });
        }
      ...
      Container(
          ...
          child: ListTile(
            trailing: IconButton(
              icon: Icon(Icons.close, color: defaultColor),
              onPressed: () {
                if (widget.newAppointment != null && widget.events.appointments.contains(widget.newAppointment)) {
                  /// To remove the created appointment, when the appointment editor
                  /// closed without saving the appointment.
                  widget.events.appointments.removeAt(widget.events.appointments.indexOf(widget.newAppointment));
                  widget.events.notifyListeners(CalendarDataSourceAction.remove, <Appointment>[widget.newAppointment]);
                }
            ...
            title: TextField(
              ...
              decoration: InputDecoration(
                ...
                focusedBorder: UnderlineInputBorder(
                    borderSide: BorderSide(color: widget.model.backgroundColor, width: 2.0, style: BorderStyle.solid)),
            ...
            child: ListTile(
              ...
              title: RawMaterialButton(
                ...
                child: Container(
                  ...
                  child: _getResourceEditor(TextStyle(fontSize: 15, color: defaultColor, fontWeight: FontWeight.w300)),
                ),
              ),
            )),
      Container(
          ...
          child: ListTile(
            leading: Container(
                width: 30,
                alignment: Alignment.centerRight,
                child: Icon(Icons.lens, size: 20, color: widget.colorCollection[_selectedColorIndex])),
            title: RawMaterialButton(
              padding: const EdgeInsets.only(left: 5),
              onPressed: () {
                showDialog<Widget>(
              ...
              children: <Widget>[
                                ...
                                onWillPop: () async {
                                  if (widget.newAppointment != null) {
                                    widget.events.appointments
                                        .removeAt(widget.events.appointments.indexOf(widget.newAppointment));
                                    widget.events.notifyListeners(
                                        CalendarDataSourceAction.remove, <Appointment>[widget.newAppointment]);
                                  }
                                  return true;
                                },
                ...
                Padding(
                  ...
                  child: RawMaterialButton(
                    ...
                    onPressed: () {
                      if (widget.selectedAppointment != null || widget.newAppointment != null) {
                        if (widget.events.appointments.isNotEmpty &&
                            widget.events.appointments.contains(widget.selectedAppointment)) {
                          widget.events.appointments
                        ...
                        if (widget.appointment.isNotEmpty && widget.appointment.contains(widget.newAppointment)) {
                          widget.appointment.removeAt(widget.appointment.indexOf(widget.newAppointment));
                        }
                      }
                      ...
                      widget.appointment.add(Appointment(
                        ...
                        color: widget.colorCollection[_selectedColorIndex],
                        startTimeZone:
                            _selectedTimeZoneIndex == 0 ? '' : widget.timeZoneCollection[_selectedTimeZoneIndex],
                        endTimeZone:
                            _selectedTimeZoneIndex == 0 ? '' : widget.timeZoneCollection[_selectedTimeZoneIndex],
                        notes: _notes,
                        isAllDay: _isAllDay,
                      ...
                      widget.events.notifyListeners(CalendarDataSourceAction.add, widget.appointment);
...
class AppointmentEditorWeb extends StatefulWidget {
  ...
  const AppointmentEditorWeb(this.model, this.selectedAppointment, this.colorCollection, this.colorNames, this.events,
      this.timeZoneCollection, this.appointment,
      [this.newAppointment]);
...
class _AppointmentEditorWebState extends State<AppointmentEditorWeb> {
  ...
  void _updateAppointmentProperties() {
    ...
    _endTime = TimeOfDay(hour: _endDate.hour, minute: _endDate.minute);
    _isTimeZoneEnabled =
        widget.selectedAppointment.startTimeZone != null && widget.selectedAppointment.startTimeZone.isNotEmpty;
    _selectedResources = _getSelectedResources(_resourceIds, widget.events.resources);
    _unSelectedResources = _getUnSelectedResources(_selectedResources, widget.events.resources);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      ...
      child: Container(
        ...
        height: widget.events.resources != null && widget.events.resources.isNotEmpty
            ? _isTimeZoneEnabled
                ? 560
                : 500
            : _isTimeZoneEnabled
        ...
        child: ListView(
          ...
          children: <Widget>[
            Container(
                ...
                child: ListTile(
                  title: Text(
                    widget.selectedAppointment != null && widget.newAppointment == null
                        ? 'Edit appointment'
                        : 'New appointment',
                    style: TextStyle(fontSize: 18, fontWeight: FontWeight.w400, color: defaultTextColor),
                  ),
                  trailing: IconButton(
                    icon: Icon(Icons.close, color: defaultColor),
                    onPressed: () {
                      if (widget.newAppointment != null && widget.events.appointments.contains(widget.newAppointment)) {
                        ...
                        widget.events.appointments.removeAt(widget.events.appointments.indexOf(widget.newAppointment));
                        widget.events
                            .notifyListeners(CalendarDataSourceAction.remove, <Appointment>[widget.newAppointment]);
                      }
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              Text(
                                'Title',
                                style: TextStyle(fontSize: 12, color: defaultColor, fontWeight: FontWeight.w300),
                                textAlign: TextAlign.start,
                              ),
                              TextField(
                                autofocus: true,
                                ...
                                keyboardType: TextInputType.multiline,
                                maxLines: null,
                                style: TextStyle(fontSize: 13, color: defaultTextColor, fontWeight: FontWeight.w400),
                                decoration: InputDecoration(
                                  isDense: true,
                                  focusColor: widget.model.backgroundColor,
                                  border: const UnderlineInputBorder(),
                                  focusedBorder: UnderlineInputBorder(
                                      borderSide: BorderSide(
                                          color: widget.model.backgroundColor, width: 2.0, style: BorderStyle.solid)),
                                ),
                              ),
                            ],
                          ),
                          ...
                          children: <Widget>[
                            ...
                            TextField(
                              ...
                              controller: TextEditingController(
                                  text: (_isAllDay ? DateFormat('dd/MM/yyyy') : DateFormat('dd/MM/yy h:mm a'))
                                      .format(_startDate)),
                              onChanged: (String value) {
            ...
            if (_isTimeZoneEnabled)
              Container(
                  child: ListTile(
                contentPadding: const EdgeInsets.only(left: 20, top: 2, bottom: 2, right: 305),
                title: Container(
                  decoration: BoxDecoration(
                    border: Border(
                      bottom: BorderSide(
                      ...
                      Expanded(
                        ...
                        child: Row(
                          ...
                          children: <Widget>[
                            Text(
                              widget.timeZoneCollection[_selectedTimeZoneIndex],
                              style: TextStyle(fontSize: 13, color: defaultColor, fontWeight: FontWeight.w400),
                            ),
                            const Icon(
                    ...
                    children: <Widget>[
                      ...
                      Container(
                        ...
                        child: _getResourceEditor(
                            TextStyle(fontSize: 13, color: defaultColor, fontWeight: FontWeight.w400)),
                      )
                    ]),
                onTap: () {
                  showDialog<Widget>(
                              ...
                              child: Row(
                                ...
                                children: <Widget>[
                                  Text(
                                    widget.colorNames[_selectedColorIndex],
                                    style:
                                        TextStyle(fontSize: 13, color: defaultTextColor, fontWeight: FontWeight.w400),
                                  ),
                                  const Icon(
                                    Icons.arrow_drop_down,
                                    size: 24,
...
class AppointmentEditor extends StatefulWidget {
  ...
  const AppointmentEditor(this.model, this.selectedAppointment, this.targetElement, this.selectedDate,
      this.colorCollection, this.colorNames, this.events, this.timeZoneCollection,
...
class _AppointmentEditorState extends State<AppointmentEditor> {
  ...
  Widget _getAppointmentEditor(BuildContext context, Color backgroundColor, Color defaultColor) {
    return Container(
                      ...
                      onTap: () async {
                        ...
                        if (date != null && date != _startDate) {
                          setState(() {
                            final Duration difference = _endDate.difference(_startDate);
                            _startDate =
                                DateTime(date.year, date.month, date.day, _startTime.hour, _startTime.minute, 0);
                            _endDate = _startDate.add(difference);
                            _endTime = TimeOfDay(hour: _endDate.hour, minute: _endDate.minute);
                          });
                        }
                              ...
                              onTap: () async {
                                ...
                                if (time != null && time != _endTime) {
                                  setState(() {
                                    ...
                                    final Duration difference = _endDate.difference(_startDate);
                                    _endDate = DateTime(
                                        _endDate.year, _endDate.month, _endDate.day, _endTime.hour, _endTime.minute, 0);
                                    if (_endDate.isBefore(_startDate)) {
                                      _startDate = _endDate.subtract(difference);
                                      _startTime = TimeOfDay(hour: _startDate.hour, minute: _startDate.minute);
                                    }
  ...
  Widget build(BuildContext context) {
    return Theme(
        data: widget.model.themeData,
        child: Scaffold(
            backgroundColor: widget.model.themeData != null && widget.model.themeData.brightness == Brightness.dark
                ? Colors.grey[850]
                : Colors.white,
            appBar: AppBar(
...
//                      context,
//                      (widget.model.themeData.brightness == Brightness.dark ? Colors.grey[850] : Colors.white),
//                      widget.model.themeData.brightness != null && widget.model.themeData.brightness == Brightness.dark
//                          ? Colors.white
//                          : Colors.black87)
                  _getAppointmentEditor(
                      context,
