...
class TableCalendar extends StatefulWidget {
  ...
  /// Use it to update `events`, `checkindays`,`checkoutdays` etc.
  ...
  /// `Map` of checkindays & checkoutdays.
  /// This property allows you to provide custom checkinday rules.
  final Map<DateTime, List> checkindays;
  final Map<DateTime, List> checkoutdays;
  final Map<DateTime, List> offdays;
  final Map<DateTime, List> endweeks;
...
class _TableCalendarState extends State<TableCalendar>
  ...
  void initState() {
    ...
    widget.calendarController._init(
      ...
      checkindays: widget.checkindays,
      offdays: widget.offdays,
      endweeks: widget.endweeks,
      checkoutdays: widget.checkoutdays,
  ...
  void didUpdateWidget(TableCalendar oldWidget) {
    ...
    if (oldWidget.checkindays != widget.checkindays) {
      widget.calendarController._checkindays = widget.checkindays;
    }
    if (oldWidget.checkoutdays != widget.checkoutdays) {
      widget.calendarController._checkoutdays = widget.checkoutdays;
    }
    if (oldWidget.offdays != widget.offdays) {
      widget.calendarController._offdays = widget.offdays;
    }
    if (oldWidget.endweeks != widget.endweeks) {
      widget.calendarController._endweeks = widget.endweeks;
    }
  }
  ...
  void _selectedDayCallback(DateTime day) {
    if (widget.onDaySelected != null) {
      widget.onDaySelected(
        ...
        widget.calendarController.visibleCheckindays[_getCheckindaysKey(day)] ??
            [],
        widget.calendarController
                .visibleCheckoutdays[_getCheckoutdaysKey(day)] ??
            [],
        widget.calendarController.visibleOffdays[_getOffdaysKey(day)] ?? [],
        widget.calendarController.visibleEndweeks[_getEndweeksKey(day)] ?? [],
      );
    }
  }
  ...
  DateTime _getCheckindaysKey(DateTime day) {
    return widget.calendarController._getCheckindaysKey(day);
  }

  DateTime _getCheckoutdaysKey(DateTime day) {
    return widget.calendarController._getCheckoutdaysKey(day);
  }

  DateTime _getOffdaysKey(DateTime day) {
    return widget.calendarController._getOffdaysKey(day);
  }

  DateTime _getEndweeksKey(DateTime day) {
    return widget.calendarController._getEndweeksKey(day);
  }
  ...
  Widget _buildTable() {
    ...
    return Table(
      ...
      defaultColumnWidth: FractionColumnWidth(1 / daysInWeek),
      children: children,
    );
  }
    ...
    return TableRow(
      ...
      children:
        ...
        final weekdayString = widget.daysOfWeekStyle.dowTextBuilder != null
            ...
            : DateFormat.E(widget.locale).format(date).substring(0, 1);
        final isWeekend =
  ...
  Widget _buildCell(DateTime date) {
    ...
    final checkindaysKey = _getCheckindaysKey(date);
    final checkoutdaysKey = _getCheckoutdaysKey(date);
    final offdaysKey = _getOffdaysKey(date);
    final endweeksKey = _getEndweeksKey(date);
    ...
    if (key != null) {
      ...
      final events = eventKey != null
          ...
          : [];
      final checkindays = checkindaysKey != null
          ? widget.calendarController.visibleCheckindays[checkindaysKey]
          : [];
      final checkoutdays = checkoutdaysKey != null
          ? widget.calendarController.visibleCheckoutdays[checkoutdaysKey]
          : [];
      final offdays = offdaysKey != null
          ? widget.calendarController.visibleOffdays[offdaysKey]
          : [];
      final endweeks = endweeksKey != null
          ? widget.calendarController.visibleEndweeks[endweeksKey]
          : [];
        ...
        if (widget.builders.markersBuilder != null) {
          ...
          children.add(
            Positioned(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: checkindays
                    .take(widget.calendarStyle.markersMaxAmount)
                    .map((checkin) => _buildMarker(checkindaysKey, checkin))
                    .toList(),
              ),
            ),
          );
  ...
  Widget _buildCellContent(DateTime date) {
    ...
    final tIsCheckindays = widget.calendarController.visibleCheckindays
        .containsKey(_getCheckindaysKey(date));
    final tIsCheckoutdays = widget.calendarController.visibleCheckoutdays
        .containsKey(_getCheckoutdaysKey(date));
    final tIsOffdays = widget.calendarController.visibleOffdays
        .containsKey(_getOffdaysKey(date));
    final tIsEndweeks = widget.calendarController.visibleEndweeks
        .containsKey(_getEndweeksKey(date));
    final tIsWeekend =
        widget.calendarController._isWeekend(date, widget.weekendDays);
    final tIsEventDay =
    ...
    final isOutsideCheckindays =
        widget.builders.outsideCheckindayDayBuilder != null &&
            tIsOutside &&
            tIsCheckindays;
    final isOutsideCheckoutdays =
        widget.builders.outsideCheckoutdayDayBuilder != null &&
            tIsOutside &&
            tIsCheckoutdays;
    final isOutsideOffdays = widget.builders.outsideOffdayDayBuilder != null &&
        tIsOutside &&
        tIsOffdays;
    final isOutsideEndweek = widget.builders.outsideEndweekDayBuilder != null &&
        tIsOutside &&
        tIsEndweeks;
    final isCheckin = widget.builders.checkinDayBuilder != null &&
        !tIsOutside &&
        tIsCheckindays;
    final isCheckout = widget.builders.checkoutDayBuilder != null &&
        !tIsOutside &&
        tIsCheckoutdays;
    final isOff =
        widget.builders.offDayBuilder != null && !tIsOutside && tIsOffdays;
    final isEndweek =
        widget.builders.endweekDayBuilder != null && !tIsOutside && tIsEndweeks;
    ...
    if (isUnavailable) {
      ...
      return widget.builders.selectedDayBuilder(
          context, date, widget.calendarController.visibleEvents[eventKey]);
    } else if (isOutsideCheckindays) {
      return widget.builders.outsideCheckindayDayBuilder(
          context, date, widget.calendarController.visibleEvents[eventKey]);
    } else if (isCheckin) {
      return widget.builders.checkinDayBuilder(
          context, date, widget.calendarController.visibleEvents[eventKey]);
    } else if (isOutsideCheckoutdays) {
      return widget.builders.outsideCheckoutdayDayBuilder(
          context, date, widget.calendarController.visibleEvents[eventKey]);
    } else if (isCheckout) {
      return widget.builders.checkoutDayBuilder(
          context, date, widget.calendarController.visibleEvents[eventKey]);
    } else if (isOutsideOffdays) {
      return widget.builders.outsideOffdayDayBuilder(
          context, date, widget.calendarController.visibleEvents[eventKey]);
    } else if (isOff) {
      return widget.builders.offDayBuilder(
          context, date, widget.calendarController.visibleEvents[eventKey]);
    } else if (isOutsideEndweek) {
      return widget.builders.outsideEndweekDayBuilder(
          context, date, widget.calendarController.visibleEvents[eventKey]);
    } else if (isEndweek) {
      return widget.builders.endweekDayBuilder(
          context, date, widget.calendarController.visibleEvents[eventKey]);
      ...
      return _CellWidget(
        ...
        isCheckin: tIsCheckindays,
        isCheckout: tIsCheckoutdays,
        isOff: tIsOffdays,
        isEnweek: tIsEndweeks,
