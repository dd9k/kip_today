...
//  ViewNavigationMode _viewNavigationMode = ViewNavigationMode.snap;
  ...
  Widget build(BuildContext context) {
    final Widget calendar = Theme(
        ...
        child: _getGettingStartedCalendar(
            _calendarController, _events, _onViewChanged, _minDate, _maxDate, scheduleViewBuilder));

    final double screenHeight = MediaQuery.of(context).size.height;
    return Scaffold(
      body: Row(children: <Widget>[
        Expanded(
          child: _calendarController.view == CalendarView.month && model.isWebFullView && screenHeight < 800
              ? Scrollbar(
                  isAlwaysShown: true,
  ...
  void _onViewChanged(ViewChangedDetails visibleDatesChangedDetails) {
    ...
    if (_calendarController.view == CalendarView.month || _calendarController.view == CalendarView.timelineMonth) {
      for (int i = 0; i < 5; i++) {
        blockedDates.add(
            visibleDatesChangedDetails.visibleDates[random.nextInt(visibleDatesChangedDetails.visibleDates.length)]);
      }
    }
    ...
    if (_calendarController.view != CalendarView.schedule) {
      for (int i = 0; i < visibleDatesChangedDetails.visibleDates.length; i++) {
        ...
        if (blockedDates != null && blockedDates.isNotEmpty && blockedDates.contains(date)) {
          continue;
        }
        ...
        children: <Widget>[
          Container(
            child: Row(
              ...
              children: <Widget>[
                Text('Allow view navigation', style: TextStyle(fontSize: 16.0, color: model.textColor)),
                Container(
                  padding: const EdgeInsets.fromLTRB(0, 0, 0, 0),
                  child: Theme(
                    data: Theme.of(context).copyWith(canvasColor: model.bottomSheetBackgroundColor),
                    child: Container(
          ...
          Container(
            child: Row(
              ...
              mainAxisSize: MainAxisSize.max,
              children: <Widget>[
                Text('Show date picker button', style: TextStyle(fontSize: 16.0, color: model.textColor)),
                Container(
          ...
          Container(
            child: Row(
              ...
              children: <Widget>[
                Expanded(
                    child:
                        Text('Show current time indicator', style: TextStyle(fontSize: 16.0, color: model.textColor))),
                Container(
                  padding: const EdgeInsets.fromLTRB(0, 0, 0, 0),
                  child: Theme(
                    data: Theme.of(context).copyWith(canvasColor: model.bottomSheetBackgroundColor),
          ...
          Container(
            child: Row(
              ...
              children: <Widget>[
                Expanded(
                    flex: 6,
                    child: Text('View navigation mode', style: TextStyle(fontSize: 16.0, color: model.textColor))),
                Expanded(
                  flex: 4,
                  child: Container(
                    ...
                    child: DropdownButton<String>(
                        ...
                        items: _viewNavigationModeList.map((String value) {
                          return DropdownMenuItem<String>(
                              ...
                              child:
                                  Text(value, textAlign: TextAlign.center, style: TextStyle(color: model.textColor)));
                        }).toList(),
...
//      showCurrentTimeIndicator: _showCurrentTimeIndicator,
      ...
      blackoutDatesTextStyle:
          TextStyle(decoration: model.isWebFullView ? null : TextDecoration.lineThrough, color: Colors.red),
      minDate: _minDate,
      maxDate: _maxDate,
