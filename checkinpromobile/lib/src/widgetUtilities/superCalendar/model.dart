...
class WidgetCategory {
  ...
  WidgetCategory([this.categoryName, this.controlList, this.mobileCategoryId, this.webCategoryId, this.showInWeb]);

  /// Getting the control details from the json file
  factory WidgetCategory.fromJson(Map<String, dynamic> json) {
    return WidgetCategory(
        json['categoryName'], json['controlList'], json['mobileCategoryId'], json['webCategoryId'], json['showInWeb']);
  }
...
class Control {
  ...
  Control(this.title, this.description, this.image, this.status, this.displayType, this.subItems, this.controlId,
      this.showInWeb, this.isBeta);

  /// Getting the control details from the json file
  factory Control.fromJson(Map<String, dynamic> json) {
    return Control(json['title'], json['description'], json['image'], json['status'], json['displayType'],
        json['subItems'], json['controlId'], json['showInWeb'], json['isBeta']);
  }
    ...
    for (int index = 0; index < controlList.length; index++) {
      if (controlList[index].sampleList != null) {
        ...
        for (int i = 0; i < controlList[index].childList?.length; i++) {
          for (int j = 0; j < controlList[index].childList[i].subItems?.length; j++) {
            if (controlList[index].childList[i].subItems[j].type != 'child') {
              searchSampleItems.add(controlList[index].childList[i].subItems[j]);
            } else {
              //ignore: prefer_foreach
              for (final SubItem sample in controlList[index].childList[i].subItems[j].subItems) {
        ...
        for (int i = 0; i < controlList[index].subItems?.length; i++) {
          for (int j = 0; j < controlList[index].subItems[i].subItems.length; j++) {
            for (int k = 0; k < controlList[index].subItems[i].subItems[j].subItems.length; k++) {
              searchSampleItems.add(controlList[index].subItems[i].subItems[j].subItems[k]);
            }
          }
        }
      }
...
//  }
  ...
  bool _isChild = false;
  final bool _isWeb = kIsWeb || Platform.isWindows || Platform.isMacOS || Platform.isLinux;
  final String _jsonText = await rootBundle.loadString('lib/sample_details.json');
  ...
  for (int index = 0; index < categoryList.length; index++) {
    ...
    if ((!_isWeb || SampleModel._categoryList[index].showInWeb != false) &&
      ...
      for (int i = 0; i < SampleModel._categoryList[index].controlList?.length; i++) {
        controlList.add(Control.fromJson(SampleModel._categoryList[index].controlList[i]));
        if (!_isWeb || controlList[i].showInWeb != false) {
          for (int j = 0; j < controlList[i].subItems?.length; j++) {
            _firstLevelSubItems.add(SubItem.fromJson(controlList[i].subItems[j]));
            if (_firstLevelSubItems[j].type == 'parent') {
              for (int k = 0; k < _firstLevelSubItems[j].subItems?.length; k++) {
                if (!_isWeb || SubItem.fromJson(_firstLevelSubItems[j].subItems[k]).showInWeb != false) {
                  _secondLevelSubItems.add(SubItem.fromJson(_firstLevelSubItems[j].subItems[k]));
                  for (int l = 0; l < _secondLevelSubItems[_secondLevelSubItems.length - 1].subItems.length; l++) {
                    if (!_isWeb ||
                        SubItem.fromJson(_secondLevelSubItems[_secondLevelSubItems.length - 1].subItems[l]).showInWeb !=
                            false) {
                      _thirdLevelSubItems
                          .add(SubItem.fromJson(_secondLevelSubItems[_secondLevelSubItems.length - 1].subItems[l]));
                    }
                    _thirdLevelSubItems[_thirdLevelSubItems.length - 1].parentIndex = j;
                    _thirdLevelSubItems[_thirdLevelSubItems.length - 1].childIndex = k;
                    _thirdLevelSubItems[_thirdLevelSubItems.length - 1].sampleIndex ??= _thirdLevelSubItems.length - 1;
                    _thirdLevelSubItems[_thirdLevelSubItems.length - 1].control = controlList[i];
                    final String breadCrumbText = ('/' +
                            controlList[i].title +
                            '/' +
                            _firstLevelSubItems[j].title +
                            '/' +
                            _secondLevelSubItems[_secondLevelSubItems.length - 1].title +
                            (_secondLevelSubItems[_secondLevelSubItems.length - 1].subItems.length == 1
                                ? ''
                                : ('/' + _thirdLevelSubItems[_thirdLevelSubItems.length - 1].title)))
                        .replaceAll(' ', '-')
                        .toLowerCase();
                    _thirdLevelSubItems[_thirdLevelSubItems.length - 1].breadCrumbText = breadCrumbText;
                    _thirdLevelSubItems[_thirdLevelSubItems.length - 1].categoryName =
                        SampleModel._categoryList[index].categoryName;
                    sampleRoutes.add(SampleRoute(
                        routeName: breadCrumbText, subItem: _thirdLevelSubItems[_thirdLevelSubItems.length - 1]));
                  }
                  _secondLevelSubItems[_secondLevelSubItems.length - 1].subItems = _thirdLevelSubItems;
              ...
              if (!_isWeb || _firstLevelSubItems[j].showInWeb != false) {
                ...
                for (int k = 0; k < _firstLevelSubItems[j].subItems?.length; k++) {
                  if (!_isWeb || SubItem.fromJson(_firstLevelSubItems[j].subItems[k]).showInWeb != false) {
                    _secondLevelSubItems.add(SubItem.fromJson(_firstLevelSubItems[j].subItems[k]));
                    _secondLevelSubItems[_secondLevelSubItems.length - 1].childIndex = j;
                    _secondLevelSubItems[_secondLevelSubItems.length - 1].sampleIndex ??= k;
                    _secondLevelSubItems[_secondLevelSubItems.length - 1].control = controlList[i];
                    final String breadCrumbText = ('/' +
                            controlList[i].title +
                            '/' +
                            _firstLevelSubItems[j].title +
                        ...
                        .replaceAll(' ', '-')
                        .toLowerCase();
                    _secondLevelSubItems[_secondLevelSubItems.length - 1].breadCrumbText = breadCrumbText;
                    _secondLevelSubItems[_secondLevelSubItems.length - 1].categoryName =
                        SampleModel._categoryList[index].categoryName;
                    sampleRoutes.add(SampleRoute(
                        routeName: breadCrumbText, subItem: _secondLevelSubItems[_secondLevelSubItems.length - 1]));
                  }
                }
                _firstLevelSubItems[j].subItems = _secondLevelSubItems;
                _secondLevelSubItems = <SubItem>[];
              ...
              if (!_isWeb || _firstLevelSubItems[j].showInWeb != false) {
                final String breadCrumbText = ('/' + controlList[i].title + '/' + _firstLevelSubItems[j].title)
                    .replaceAll(' ', '-')
                    .toLowerCase();
                _firstLevelSubItems[j].breadCrumbText = breadCrumbText;
                _firstLevelSubItems[j].control = controlList[i];
                _firstLevelSubItems[j].categoryName = SampleModel._categoryList[index].categoryName;
                sampleRoutes.add(SampleRoute(routeName: breadCrumbText, subItem: _firstLevelSubItems[j]));
          ...
          (!_isSample) ? controlList[i].subItems = _firstLevelSubItems : _isSample = false;
  ...
  for (int i = 0; i < SampleModel._categoryList.length; i++) {
    SampleModel._categoryList[i].controlList?.sort((dynamic a, dynamic b) => a.controlId.compareTo(b.controlId) as int);
  }

  if (_isWeb) {
    /// Sorting categories based on [webCategoryId]
    SampleModel._categoryList.sort((WidgetCategory a, WidgetCategory b) => a.webCategoryId?.compareTo(b.webCategoryId));
  } else {
    /// Sorting categories based on [mobileCategoryId]
    SampleModel._categoryList
        .sort((WidgetCategory a, WidgetCategory b) => a.mobileCategoryId?.compareTo(b.mobileCategoryId));
  }
}
