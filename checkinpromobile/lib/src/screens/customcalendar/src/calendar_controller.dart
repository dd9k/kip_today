...
class CalendarController {
  ...
  /// `Map` of currently visible checkindays.
  Map<DateTime, List> get visibleCheckindays {
    if (_checkindays == null) {
      return {};
    }
    return Map.fromEntries(
      _checkindays.entries.where((entry) {
        for (final day in visibleDays) {
          if (_isSameDay(day, entry.key)) {
            return true;
          }
  ...
  /// `Map` of currently visible checkoutdays.
  Map<DateTime, List> get visibleCheckoutdays {
    if (_checkoutdays == null) {
      return {};
    }
    return Map.fromEntries(
      _checkoutdays.entries.where((entry) {
        for (final day in visibleDays) {
          if (_isSameDay(day, entry.key)) {
            return true;
          }
  ...
  Map<DateTime, List> get visibleOffdays {
    if (_offdays == null) {
      return {};
    }
    return Map.fromEntries(
      _offdays.entries.where((entry) {
        for (final day in visibleDays) {
          if (_isSameDay(day, entry.key)) {
            return true;
          }
  ...
  Map<DateTime, List> get visibleEndweeks {
    if (_endweeks == null) {
      return {};
    }
    return Map.fromEntries(
      _endweeks.entries.where((entry) {
        for (final day in visibleDays) {
          if (_isSameDay(day, entry.key)) {
            return true;
          }
  ...
  Map<DateTime, List> _checkindays;
  Map<DateTime, List> _checkoutdays;
  Map<DateTime, List> _offdays;
  Map<DateTime, List> _endweeks;
  ...
  void _init({
    ...
    @required Map<DateTime, List> checkindays,
    @required Map<DateTime, List> checkoutdays,
    @required Map<DateTime, List> offdays,
    @required Map<DateTime, List> endweeks,
  ...
  DateTime _getCheckindaysKey(DateTime day) {
    return visibleCheckindays.keys
        .firstWhere((it) => _isSameDay(it, day), orElse: () => null);
  }

  DateTime _getCheckoutdaysKey(DateTime day) {
    return visibleCheckoutdays.keys
        .firstWhere((it) => _isSameDay(it, day), orElse: () => null);
  }

  DateTime _getOffdaysKey(DateTime day) {
    return visibleOffdays.keys
        .firstWhere((it) => _isSameDay(it, day), orElse: () => null);
  }
DateTime _getEndweeksKey(DateTime day) {
    return visibleEndweeks.keys
        .firstWhere((it) => _isSameDay(it, day), orElse: () => null);
  }
